...
assert.doesNotMatch(
  pinDetail,
  /COMMENT_COMPOSER_ACTIVE_CLASS|pin-comment-composer-active|setCommentComposerLayerMode/,
  "comment composer should not toggle a root class that changes the detail sheet material during focus/blur",
);
assert.doesNotMatch(
  pinDetail,
  /commentComposerReleasePending|commentComposerReleaseTimer|visualViewport/,
  "comment composer should not run delayed keyboard layer release logic that can make the modal jump on blur",
);
assert.doesNotMatch(
  pinDetail,
  /onPointerDown=\{handleCommentComposerPointerDown\}|onFocus=\{handleCommentComposerFocus\}|onBlur=\{handleCommentComposerBlur\}/,
  "comment composer should rely on stable textarea styling instead of JS focus/blur material toggles",
);
assert.match(
  pinDetail,
...
assert.doesNotMatch(
  styles,
  /html\.pin-comment-composer-active\s+\.sheet-backdrop\.lg-overlay-backdrop/,
  "comment focus should not change the overlay/backdrop material",
);
assert.doesNotMatch(
  styles,
  /html\.pin-comment-composer-active\s+\.sheet:has\(\.pin-detail\)|html\.pin-comment-composer-active\s+\.pin-comment-main/,
  "comment focus should not remove the detail/comment glass layers",
);
assert.doesNotMatch(
  styles,
  /pin-comment-composer-active[\s\S]{0,260}(backdrop-filter:\s*none|display:\s*none|--surface-blur:\s*none)/,
  "comment focus must not make the detail background look thinner by disabling blur or decorative layers",
);
