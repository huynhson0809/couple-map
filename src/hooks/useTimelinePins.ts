...
function applyFilters(
  ...
  let next = query
    ...
    .order('sort_order', { foreignTable: 'pin_images', ascending: true })
  ...
  if (address) {
    next = next.or(`address.ilike.%${address}%,city.ilike.%${address}%,country.ilike.%${address}%`)
  }
  ...
  const fetchPage = useCallback(
    async (offset: number, append: boolean) => {
      ...
      if (error) {
        ...
        setPins((prev) => (append ? [...prev, ...((data as Pin[]) ?? [])] : ((data as Pin[]) ?? [])))
        setTotal(count ?? 0)
      }
